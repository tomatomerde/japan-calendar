...
npm で本パッケージより先に出てくる祝日ライブラリ4本は「祝日かどうか」を判定して
そこで止まるが、このライブラリは
...
ことを明示する）。4本のどれも、この区別を付けていない——
次の節で、それぞれの名前と返り値を挙げてある。
...
## npm にある他の選択肢と、それぞれがどこで止まるか

npm で日本の祝日を探すと、このパッケージより先に4本が出てくる。どれも「答える範囲では
問題なく答える」ライブラリで、足りるかどうかは、本ライブラリが**おまけではなく本体**として
扱っている2点——**営業日の計算**と、**その祝日の日付がまだ確定していないと言うこと**——を
使うかどうかで決まる。

| パッケージ | どこで止まるか |
| --- | --- |
| [`japanese-holidays`](https://www.npmjs.com/package/japanese-holidays) 1.0.10 | 祝日判定のみ。`isHoliday` は祝日**名の文字列**を返すので、「この日付は予測値」というフラグを載せる場所が原理的に無い |
| [`@holiday-jp/holiday_jp`](https://www.npmjs.com/package/@holiday-jp/holiday_jp) 2.5.1 | 祝日判定のみ。エントリが持つのは `date` / `week` / `name` / `name_en` で、確度に関する情報は無い。データは2050年で終わり、それ以降は `[]` を返す |
| [`date-holidays`](https://www.npmjs.com/package/date-holidays) 3.35.0 | 約200カ国対応が存在理由であり、本パッケージが決して持たないもの。ただし日本については、営業日計算は無く（`getDayOff()` が返すのは週の休業曜日名で、日付演算ではない）、確度のフィールドも無い |
| [`@gahojin-inc/holiday-japanese`](https://www.npmjs.com/package/@gahojin-inc/holiday-japanese) 2026.7.3 | 祝日判定のみ。`isHoliday` は真偽値を返す。こちらもデータは2050年で終わり、`isHoliday(new Date(2051, 0, 1))` は元日なのに `false` |
| [`@smarthr/wareki`](https://www.npmjs.com/package/@smarthr/wareki) 1.3.1 | 和暦のみ（祝日は扱わない）。改暦の境界での挙動は[下記](#和暦と1873年の改暦) |

### 春分・秋分は予測値であり、それを言うのは1本だけ

春分の日・秋分の日は、前年2月に官報が翌年の「暦要項」を公示するまで法的に確定しない。
同梱している公式データが確定させているのは **2027年**まで。それ以降の春分・秋分は
公示された値ではなく計算値になる。2050年3月20日を訊いたときの答え:

| | 返り値 | 未確定だと言うか |
| --- | --- | --- |
| `japanese-holidays` | `'春分の日'` | 言わない |
| `@holiday-jp/holiday_jp` | `{ name: '春分の日', … }` | 言わない |
| `date-holidays` | `{ name: '春分の日', … }` | 言わない |
| `@gahojin-inc/holiday-japanese` | `true` | 言わない |
| `japan-calendar` | `{ name: '春分の日', confirmed: false, … }` | **言う** |

**違う日付を計算している者はいない。** 2028〜2099年の春分・秋分144件すべてについて、
そこまでデータを持っている2本——`japanese-holidays` と `date-holidays`——は本パッケージと
まったく同じ日を答える。違うのは、2050年の支払処理を組む呼び出し側が、**その答えが予測で
あることを知れるかどうか**だけ。

### データが尽きる場所

残る2本は2050年で止まる。しかも**黙って**止まる:
...
isHoliday('2100-01-01'); // → OutOfRangeError を投げる
```

2051年の元日は争いようがない。春分・秋分以外の祝日は法律で日付が決まっている。
それでも呼び出し側は、`false` から「調べた結果、祝日ではない」と「その年のデータを
持っていない」を区別できない。本パッケージが2099年まで答えてそこから先を拒否するのは、
祝日ライブラリにとって危険な答えはエラーではなく**自信に満ちた `false`** だから。

これは「営業日ループくらい自分で書けるのでは」への正直な答えでもある。**書ける。**
どのライブラリの上でも十数行で、数えて通る期間のデータをその ライブラリが持っている
かぎり、本パッケージと同じ答えになる。**同じでなくなるのは、下のデータが尽きる場所。**
`2050-12-30` の3営業日後は `2051-01-05`——12/31が土曜、1/1が元日、1/2がその振替休日
なので。データが2050年で切れているライブラリの上に書いたループは、最後の2日を平日として
数えて `2051-01-04` に着く。

### 和暦と1873年の改暦

`@smarthr/wareki` は西暦→和暦文字列の変換で、本パッケージの担当範囲と半分重なる。
境界では存在しない日付を作る:
...
toWareki('1872-12-31');
// → UnsupportedWarekiRangeError を投げる
```

明治5年12月31日は存在しない。改暦ノ布告により明治5年12月3日が明治6年1月1日とされたので、
明治5年12月は2日で終わっている。改暦前は太陰太陽暦で、そもそも西暦の日付と1対1に
対応しない——本パッケージが整形せず拒否するのはそのため。

各パッケージが何を公開しているか、上の日付に何を返すか、和暦の結果——これらは固定した
バージョンに対して CI で測り直している
（[`test/alternatives.test.ts`](./test/alternatives.test.ts)）。一度きりの調査からの
引き写しではない。
