...
<!-- 以下は dev-standards の common/CLAUDE.common.md から自動同期（@ f3bfd22）。マーカーの内側を手で編集しないこと -->
...
- **CI が緑になったらマージする。** 1人運用で `required_approving_review_count` は 0 なので、
  待つべき承認は存在しない。赤ければ直す。**マージ済みの**作業ブランチはそのまま消してよい
  （中身が main に入っていることを `git cherry` などで確かめてから消すこと。未マージの
  ブランチは下の「止まってよい」の2番に当たる）
...
2. **取り消せない破壊的操作。** 公開済みバージョンの unpublish、リポジトリの削除、
   **未マージのブランチの削除**、履歴の書き換え、`main` への force push、公開済みデータの削除。
   ...
   あるので復元できる——これは破壊的操作ではなく、上の自動で進める側に入る
