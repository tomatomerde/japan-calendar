...
<!-- 以下は dev-standards の common/CLAUDE.common.md から自動同期（@ 497a369）。マーカーの内側を手で編集しないこと -->
...
- **CI が緑になったらマージする。** 1人運用で `required_approving_review_count` は 0 なので、
  待つべき承認は存在しない。赤ければ直す。マージ後、不要になった作業ブランチも消す
...
2. **取り消せない破壊的操作。** 公開済みバージョンの unpublish、リポジトリやブランチの削除、
   履歴の書き換え、`main` への force push、公開済みデータの削除
