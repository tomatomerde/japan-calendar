...
<!-- 以下は dev-standards の common/CLAUDE.common.md から自動同期（@ 3f3d135）。マーカーの内側を手で編集しないこと -->
...
- **同じ理由で、PR・イシュー・コメントの本文にもセッション URL を書かない。**
  ...
  **書かないだけでは足りない。** 実行環境は PR を**作成した時点で**
  `_Generated by [Claude Code](https://claude.ai/code/session_…)_` を自動で
  付けることがある（2026-08-11 に実測）。したがって **PR を作ったら本文を
  1度編集して落とす**。編集のときに付き直すフッターは
  `https://claude.ai/code`——**セッション ID を含まない版**なので、そこで止まる。
  そちらは誰でも開けるので実害がなく、消しても次の書き込みで戻るだけなので
  追いかけないこと。落とすべきなのは `session_` を含む URL だけ
...
**「やっていいですか」と聞いて止まるのは、原則として禁止。** 手順として定まっているもの・
このファイルが推奨しているものは、聞かずに実行して、やった結果を報告する。
確認を挟むのは作業を安全にしないし、依頼者の手を止めるぶんだけ確実に損をする。
判断に迷ったら「止まって聞く」ではなく「決めて、根拠と一緒に報告する」。

聞かずに最後までやること:

- **PR を作る。** ブランチに push したら PR まで作るのが完了の定義（「作業の残し方」参照）。
  作ってよいか確認しない。作成後は本文からセッション URL を落とすところまで含めて1続き
- **CI が緑になったらマージする。** 1人運用で `required_approving_review_count` は 0 なので、
  待つべき承認は存在しない。赤ければ直す。**マージ済みの作業ブランチは放っておいてよい**——
  各リポジトリで head branch の自動削除が有効で、マージした時点で消える（2026-08-12 に
  4案件で実測。手で消そうとすると `remote ref does not exist` になる）。手で消す必要が
  あるのは自動削除が効かなかったときだけで、そのとき**マージ済みかどうかは PR の状態で
  判断する。`git cherry` や `git diff` で判定しないこと**——squash merge はブランチの
  コミット群を1つのパッチにまとめるので、元のコミットは main に入ったあとも永久に
  「未マージ」に見える。未マージのブランチは下の「止まってよい」の2番に当たる
- **リリースを出す。** バージョンと CHANGELOG が整合していて、`docs/releasing.md` の手順に
  沿っているなら、タグを押すところまで進める。リリースワークフロー自体が公開前の検査を
  すべて持っているので、そこが最後の砦になる
- **同期 PR・依存更新・軽微な修正**も同様。中身を確認して問題なければマージまで

**それでも止まってよい（止まるべき）のは次の4つだけ:**

1. **自分で実行できないもの。** 権限・課金・アカウント設定・シークレットの削除など。
   これは「確認」ではなくブロッカーなので、下の「人間待ちの扱い」に従って渡す
2. **取り消せない破壊的操作。** 公開済みバージョンの unpublish、リポジトリの削除、
   **未マージのブランチの削除**、履歴の書き換え、`main` への force push、公開済みデータの削除。
   基準は「消したものを手元の情報だけで復元できるか」。マージ済みブランチは main に中身が
   あるので復元できる——これは破壊的操作ではない（そもそも自動削除されるので出番がない）
3. **依頼の前提が崩れているとき。** 直そうとしている不具合が再現しない、指示された変更が
   既に入っている、など。そのまま進めても無意味なので、事実を報告して指示を仰ぐ
4. **設計上の分岐で、どちらを選んでも後から高くつくとき。** パッケージ名、公開 API の形、
   ライセンスなど、あとで変えられないもの。これは「確認」ではなく設計判断の相談

**逆に、これらは止まる理由にならない:** 変更が大きいこと、複数のリポジトリにまたがること、
自信が完全ではないこと、依頼者が見ていない時間帯であること。**大きい変更ほど、聞くのではなく
検証を厚くして進める。**
