...
5. **publish が通ったら `demo/pinned-version.txt` を新しいバージョンに上げて push する。**
   デモページはレジストリの公開版を固定して読み込むので、上げないと**古い版の挙動を
   見せ続ける**。忘れても赤くならない——`Demo (GitHub Pages)` は `demo/**` に
   `paths:` フィルタが掛かっていて、リリースのコミットでは起動しないため。
   これを拾うのが同ワークフローの週次 `pin-freshness` ジョブで、月曜に赤くなる。
   pin を上げるコミットが `demo/**` に触るので、そのまま build → 検証 → デプロイまで走る。
