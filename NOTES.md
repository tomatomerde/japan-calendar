...
最終更新: 2026-08-05（PR #5 作成。drift ワークフローの原本同期と、
dev-standards への昇格提案を open 項目として追加）
...
**進行中: PR #5**（ブランチ `claude/sync-dev-standards-pr4-66gd0u`）。
`.github/workflows/check-claude-md-drift.yml` を dev-standards の
`examples/check-claude-md-drift.yml` とバイト単位で一致させただけ（差分は
外部コントリビュータ向け注記6行）。CI は7ジョブすべて green、マージ可能。
**ただし同期先は原本 `399e3e5` 版で、既に1世代古い**（下記の追随②を参照）。

**`CLAUDE.md` の共通部分は原本 `03e7e41` と一致済み**（`399e3e5` と同一ツリー。
dev-standards の PR #5 は PR #4 と同じ v7 bump を並行して行ったため空マージに
なっている）。原本 SHA を `399e3e5` へ「更新」する指示を受けても従わないこと。
参照先が1コミット古くなるだけになる。

### dev-standards への追随2件（原本のマージ待ち。まとめて1つの PR でやる）

固有セクション「検証」の2項目を原本へ昇格させる提案は、dev-standards 側の
レビューで**①②③とも採用**された。経緯は PR #5 の2つのコメントにある
（[提案](https://github.com/tomatomerde/japan-calendar/pull/5#issuecomment-5191488552) /
[回答](https://github.com/tomatomerde/japan-calendar/pull/5#issuecomment-5192697574)）。

**どちらも原本側の PR がマージされるまで着手しない。** 先にやると原本と
食い違った状態を自分で作ることになる。2件は同じファイル群を触らないので、
両方マージされてから1つの PR にまとめるのが無駄がない。

**① 昇格に伴う重複の解消**（dev-standards#7 のマージ後）

<https://github.com/tomatomerde/dev-standards/pull/7>

`CLAUDE.md` の「このプロジェクト固有の作業ルール > 検証」に対して:

1. 「期待値を検証対象そのものから作らない」と「設定ファイルに書いたコマンドその
   ものを実行する」の2項目を**削除**（原本が持つようになるため）
2. 「共通部の『実際に見つかった不具合』**4例**は……」を **5例** に更新。
   `mkdir` の実例が5例目として原本のリストに入るため
3. 「確認手段が塞がれているなら、推測で変更せず申し送る」は原本に相当物がない。
   **残すこと**

**② drift ワークフローの再同期**（dev-standards#8 のマージ後）

<https://github.com/tomatomerde/dev-standards/pull/8>

PR #5 が同期した原本 `399e3e5` 版のあと、原本は #6 で同じファイルの注記を
書き直しており（先頭に別の注記を追加、スキップ通知を日英併記に変更）、結果
コントリビュータ向け注記が2つ並んでいる。#8 はその重複を1つに統合する。
**#8 がマージされてから `examples/check-claude-md-drift.yml` を丸ごとコピー
し直す**（#6 時点で追随すると、すぐ #8 でまた変わる）。

なお itaiji-library / jp-address-romaji は①②相当の記述を持たないことを
dev-standards 側で確認済み。①の後始末が要るのはこのリポジトリだけ。
