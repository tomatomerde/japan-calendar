...
最終更新: 2026-08-12（公開後の詳細レビューを実施。下の「公開後レビューの結果」参照）

**npm 公開済み**: `japan-calendar@0.1.1`（provenance attestation 付き）。0.1.2 を準備中。
...
## 公開後レビューの結果（2026-08-12・別セッションで実施）

公開済み 0.1.1 の実物（npm tarball）に対する詳細レビュー。見た範囲と結果:
...
- **provenance 署名の検証**（`npm audit signatures`）。開発環境のプロキシが鍵の取得を
  遮断するため実行できなかった。ネットワークの素直な環境で1回流す価値はある
