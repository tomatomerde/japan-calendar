...
要る状況がまだ起きていない、というのが正確。`0.1.2` でも同じく 12.0.2 のまま通った。

`0.1.2` は **`cut_release: true` の dispatch 経路の初の実運用**（#41 で追加された、
セッションからタグ push なしでリリースを切る経路）。publish は通り、タグ `v0.1.2` と
GitHub Release は publish 後に `gh release create --target` がまとめて作った。
レジストリ上の `0.1.2` に SLSA provenance の attestation が付いていることを
`npm view japan-calendar@0.1.2 dist` で確認済み。**この経路では provenance の ref が
`refs/heads/main`** になる（タグ push なら `refs/tags/v…`）。指すコミットは同じ。
...
## 公開後レビューの結果（2026-08-12〜13・別セッションで実施）

公開済み 0.1.1 の実物（npm tarball）に対する詳細レビュー。**指摘の修正は
`0.1.2` として公開済み**（#40）。見た範囲と結果:

- **公開物**: tarball 105ファイルを目視（意図しない同梱なし）。`dist` はローカルの
  再ビルドとバイト一致。`@arethetypeswrong/cli` 4項目 green。リポジトリ public・
  ライセンス・topics・badge リンク（ci.yml 実在）確認
- **README のコード例**: 英・日の全コード例＋境界値を、レジストリからインストールした
  実物に対して48項目実行し、**全件が記載どおり**（正常系・失敗系・エラー型・200字上限とも）
- **見つけて直したもの**（CHANGELOG の Unreleased 参照）: `ERAS` が凍結されておらず
  1行の代入でプロセス全体の和暦変換が壊せた（0.1.0 の凍結修正の取り残し）。
  Worker が未来日の和暦を 30日 immutable でキャッシュさせていた。`GET /` も同様。
  package-lock.json の version が 0.0.0 のまま。`OfficialMeta.fetchedAt` の JSDoc が旧義
- **設計判断として保留**（急ぎではない）: 和暦変換に上限年が無く、`toWareki('9999-12-31')`
  が令和7981年を返す。上限を設ける（例: 2099 で `OutOfRangeError`）のは公開 API の
  破壊的変更なので実施せず、README への明記＋Worker の short キャッシュで対応した。
  上限を入れたければ 0.x のうちに
