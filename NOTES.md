...
### dev-standards への昇格提案（返答待ち）

固有セクション「検証」の2項目——**「期待値を検証対象そのものから作らない」**と
**「設定ファイルに書いたコマンドそのものを実行する」**——を原本の共通部分へ
昇格させる提案を出してある。文案・重複分析・採用時の後始末は PR #5 のコメントに
まとめてある:

<https://github.com/tomatomerde/japan-calendar/pull/5#issuecomment-5191488552>

判断は dev-standards 側のレビューセッションで行う。**採用されたら**、この
リポジトリの固有セクションから該当2項目を削除する後始末が必要になる
（後者は原本の既存項目「CI・パッケージング・デプロイなど外部の仕組みは……」と
重複するため、原本側は追加ではなく差し替えを提案してある）。
採否が返るまでこのリポジトリ側では何もしない。
