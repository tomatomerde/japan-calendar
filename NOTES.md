...
## 解決済み（第13回レビュー: 防御的コードが「守れているか」を監視可能にした）

`holidaysForYear` は振替休日・国民の休日の導出時に前後年の法定祝日も材料に
入れている。コメントは「年境界をまたぐ連鎖は実際には起きないが、単年に
閉じていると仮定しないための防御」と説明している。

変異テストで `[year - 1, year, year + 1]` を `[year]` に縮めても**どのテストも
落ちなかった**ため、主張の真偽を実測で確認した:
**全151年の出力が完全一致**（差分0）。つまりコメントの主張は正しく、
防御は現時点で確かに冗長。

ただし「テストできない防御コード」を放置すると、将来ルール変更で必要に
なった瞬間に誰も気づけない。→ `test/invariants.test.ts` に
「当年の法定祝日だけから導出した結果 == 前後年込みの結果」を全年で検証する
テストを追加した。年をまたぐ導出が発生するようになるとこのテストが落ち、
防御コードが効き始めたことが分かる。

検証: 12/30 を祝日として追加する変異を入れると、81年ぶんで差分が出て
正しく検出することを確認済み。
