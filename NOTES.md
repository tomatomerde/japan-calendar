...
1. `CLAUDE.md` を dev-standards の共通テンプレート
   ...
   **「ここから下は共通」以降は原本と一字一句同一に保つこと。**
   プロジェクト固有のルールは先頭セクションに置く
2. `actions/checkout` と `actions/setup-node` を v4 → v7
3. 引数検証と、エラーメッセージへの入力反射の修正（下記）

### 引数検証まわりで直したこと（再発させないための要点）

パッケージを実際に作って利用者として叩いたところ、**日付以外の引数が
まったく検証されておらず、例外ではなく「それらしい間違った答え」を
返していた**。`'Bank'` が national の答えになる、`NaN` 日数が「変化なし」に
なる、`holidaysForYear(NaN)` が存在しない年の祝日を計算して記憶する、など。
あわせて、エラーメッセージが呼び出し側の入力を無制限に反射していた
（50KB のクエリパラメータが 400 の本文に丸ごと返る）。

いずれも `CONTRIBUTING.md` の Core invariants に項目として書いた。
ガードは全て**外すとテストが落ちること**を確認済み。
